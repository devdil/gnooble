...
if (Authenticate::getUserType() == "ADMIN")
{
	...
	if (!empty($_POST['sourcecode']))
	{
		...
		$lengthOfSourceCode = strlen($_POST['sourcecode']);
		 $type = $_POST['type'];
...
//		$compileMessage   = $result->getCompileMessage();
	...
	for( $index = 0 ; $index < count($isPassed); $index++)
		...
		if ($areAllPassed)
		{
			...
			// if the user hasn't solved the question then update the scoreboard
				...
				if (!$isSolved) {
					if ($type === 'cgf')
					{
					Student::updateMyCustomScoreBoard($_GET['qid'], $_SESSION['userid'], "Solved", $sourceCode, $solvedTime, $avgTime, $avgMem,$lengthOfSourceCode);
		...
		else
		{
			...
			if ($type === 'cgf')
			{
				Student::updateMyCustomScoreBoard($_GET['qid'], $_SESSION['userid'], "Attempted", $sourceCode, " '0000-00-00 00:00:00'", "NA","NA",$lengthOfSourceCode);
